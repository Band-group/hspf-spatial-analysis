...
#Andre modifications for testing##################
#args = parse_arguments()#uncomment when ready

#arguments
args <- list()
args$threads <- 4
args$grid <- "output/grids/grid-type=hexagon-size=1-division=none-area=global.rds"
args$pf_aggregated <- "output/Pf/aggregated/grid-type=hexagon-size=1-division=none-area=global.tsv"
args$HbS_aggregated <- "output/HbS/fixed-r0=25.0-sigma0=0.6-fc=none/aggregated/grid-type=hexagon-size=1-division=none-area=global.tsv"
args$sources <- NULL
args$min_N <- 5
args$locus <- "Pfsa1"
args$areas <- NULL
...
args$posterior_samples_per_hbs_sample <- 25
args$model <- "bym2"
args$size <- "1"
args$type <- "hexagon"
args$r0 <- "25.0"
